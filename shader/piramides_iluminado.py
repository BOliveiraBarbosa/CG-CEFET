...
def draw():
	...
	for i in range(0, n):
		glNormal3fv(calculaNormalFace((vertices[i][0], vertices[i][1], 0.0),
									  (-raio, -raio, h),
									  (vertices[(i+1) % n][0], vertices[(i+1) % n][1], 0.0)))
									  
		glVertex3f(vertices[i][0], vertices[i][1], 0.0)
		glVertex3f(-raio, -raio, h)
		glVertex3f(vertices[(i+1) % n][0], vertices[(i+1) % n][1], 0.0)
	glEnd()
